...
export default function StateComponent() {
    ...
    const onCountAddHandler = () => {
        ...
        // 상태가 변하지 않음. 상태변수는 반드시 상태변경함수로 변경해야 리렌더링 됨
        // count++;
        // console.log(count);

        // 상태변경 함수를 통해서 함수를 변경한다고 바로 적용되지 않음
        // 리렌더링된 후 상태변경 함수가 적용됨(함수 전체 작업이 끝난 후 setCount()가 실행됨?)
        // setCount(count + 1);
        // console.log(count); // f12에 출력됨
        // setCount(count + 1);
        // console.log(count);
        // setCount(count + 1); // 마지막 setCount()로 덮어써짐
        // console.log(count);

        // 상태변경 함수에 콜백 함수를 전달하면 해당 콜백 함수는 상태 변경 작업을 누적해서 함
        // setCount(count + 1); // -> 맨 위에 있으면 4
        // setCount((count) => count + 1);
        // setCount((count) => count + 1);
        // setCount((count) => count + 1);
        // setCount(count + 1); // 마지막에 있으면 1
    ...
    // Input 요소의 값을 가져오고자 할때 onChange 이벤트의 .target.value로 가져옴
