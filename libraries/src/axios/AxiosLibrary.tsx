...
export default function AxiosLibrary() {
    ...
    axios
        .get(GET_URL, {headers: {'Authorization' : 'Bearer 4567854'}})
        // 각각의 http method 함수 결과를 then으로  받을 땐 response 객체를 매개변수로 받는 콜백함수를 전달
        .then(response => {
            // response 객체는 status, header, data(body)를 포함하고 있음
            // then 작업은 status code가 400, 500번 대가 아닌 response를 받아옴
            console.log(response);
        })
        .catch(error => {
            // 해당 request에 대해서 request가 실패하거나 status code가 400, 500번대이면 catch로 AxiosError 객체를 받음
